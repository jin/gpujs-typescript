...
(function (Scene) {
    ...
    var generateRandomSpheres = function (count) {
        ...
        for (var i = 0; i < count; i++) {
            ary.push({
                ...
                x: rand(-7, 7), y: rand(0, 7), z: rand(-7, 2), radius: rand(1, 1),
    ...
    var opts = generateRandomSpheres(15)
        .concat(light_opts);
    ...
    var nextTick = function () {
        ...
        if (mode == Mode.CPU) {
            ...
            for (var i = 0; i < gpuKernels.length; i++) {
                var gpuKernel_1 = gpuKernels[i];
                if (bm.isBenchmarking) {
                    startTime = performance.now();
                }
                gpuKernel_1(camera, lights, entities, eyeVector, vpRight, vpUp, canvasHeight, canvasWidth, fovRadians, heightWidthRatio, halfWidth, halfHeight, cameraWidth, cameraHeight, pixelWidth, pixelHeight, i % 2, Math.floor(i / 2));
                if (bm.isBenchmarking) {
                    endTime = performance.now();
                }
                if (canvasNeedsUpdate) {
                    ...
                    var bdy = cv.parentNode;
                    var newCanvas = gpuKernel_1.getCanvas();
                    bdy.replaceChild(newCanvas, cv);
                }
            }
        }
