...
(function (Scene) {
    ...
    var opts = generateRandomSpheres(8)
        .concat(light_opts);
    ...
    var nextTick = function () {
        ...
        if (mode == Mode.CPU) {
            ...
            for (var i = 0; i < cpuKernels.length; i++) {
                ...
                cpuKernel_1(camera, lights, entities, eyeVector, vpRight, vpUp, canvasHeight, canvasWidth, fovRadians, heightWidthRatio, halfWidth, halfHeight, cameraWidth, cameraHeight, pixelWidth, pixelHeight, i % 2, Math.floor(i / 2));
            ...
            for (var i = 0; i < gpuKernels.length; i++) {
                ...
                gpuKernel(camera, lights, entities, eyeVector, vpRight, vpUp, canvasHeight, canvasWidth, fovRadians, heightWidthRatio, halfWidth, halfHeight, cameraWidth, cameraHeight, pixelWidth, pixelHeight, i % 2, Math.floor(i / 2));
                if (canvasNeedsUpdate) {
                    var cv = document.getElementById("canvas" + i).childNodes[0];
                    var bdy = cv.parentNode;
