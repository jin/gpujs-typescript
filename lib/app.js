GPU = (function () {
    var parser = (function () {
        var parser = { trace: function trace() { },
            ...
            parseError: function parseError(str, hash) { if (hash.recoverable) {
                this.trace(str);
            }
            else {
                throw new Error(str);
            } },
            parse: function parse(input) { var self = this, stack = [0], vstack = [null], lstack = [], table = this.table, yytext = "", yylineno = 0, yyleng = 0, recovering = 0, TERROR = 2, EOF = 1; var args = lstack.slice.call(arguments, 1); this.lexer.setInput(input); this.lexer.yy = this.yy; this.yy.lexer = this.lexer; this.yy.parser = this; if (typeof this.lexer.yylloc == "undefined") {
                this.lexer.yylloc = {};
            ...
            } function popStack(n) { stack.length = stack.length - 2 * n; vstack.length = vstack.length - n; lstack.length = lstack.length - n; } function lex() { var token; token = self.lexer.lex() || EOF; if (typeof token !== "number") {
                token = self.symbols_[token] || token;
            } return token; } var symbol, preErrorSymbol, state, action, a, r, yyval = {}, p, len, newState, expected; while (true) {
                ...
                _handle_error: if (typeof action === "undefined" || !action.length || !action[0]) {
                    ...
                    var errStr = "";
                    function locateNearestErrorRecoveryRule(state) { var stack_probe = stack.length - 1; var depth = 0; for (;;) {
        ...
        var lexer = (function () {
            var lexer = {
                EOF: 1,
                parseError: function parseError(str, hash) { if (this.yy.parser) {
                    this.yy.parser.parseError(str, hash);
                }
                else {
                    throw new Error(str);
                } },
                setInput: function (input) { this._input = input; this._more = this._backtrack = this.done = false; this.yylineno = this.yyleng = 0; this.yytext = this.matched = this.match = ""; this.conditionStack = ["INITIAL"]; this.yylloc = { first_line: 1, first_column: 0, last_line: 1, last_column: 0 }; if (this.options.ranges) {
                ...
                reject: function () { if (this.options.backtrack_lexer) {
                ...
                upcomingInput: function () { var next = this.match; if (next.length < 20) {
                ...
                test_match: function (match, indexed_rule) { var token, lines, backup; if (this.options.backtrack_lexer) {
                ...
                lex: function lex() { var r = this.next(); if (r) {
                    return r;
                }
                else {
                    return this.lex();
                ...
                popState: function popState() { var n = this.conditionStack.length - 1; if (n > 0) {
                    return this.conditionStack.pop();
                }
                else {
                    return this.conditionStack[0];
                } },
                _currentRules: function _currentRules() { if (this.conditionStack.length && this.conditionStack[this.conditionStack.length - 1]) {
                    return this.conditions[this.conditionStack[this.conditionStack.length - 1]].rules;
                }
                else {
                    return this.conditions["INITIAL"].rules;
                } },
                topState: function topState(n) { n = this.conditionStack.length - 1 - Math.abs(n || 0); if (n >= 0) {
    ...
    var GPUUtils = (function () {
        ...
        function init_webgl(canvasObj) {
            ...
            var webgl = (canvasObj.getContext("experimental-webgl", init_webgl_defaultOptions) ||
    ...
    var GPU = (function () {
        ...
        function createKernel(kernel, paramObj) {
            ...
            this._kernelSynchronousExecutor = ret;
            return ret;
        }
        ;
        ...
        function ast_MemberExpression_unroll(ast, funcParam) {
            ...
            if (ast.type == "MemberExpression") {
                if (ast.object && ast.property) {
                    return (ast_MemberExpression_unroll(ast.object, funcParam) +
                        "." +
        ...
        function traceFunctionCalls(functionName, retList, opt) {
            ...
            if (fNode) {
                if (retList.indexOf(functionName) >= 0) {
                    ...
                    fNode.getWebglFunctionString(opt);
...
(function (Scene) {
    ...
    var generateRandomSpheres = function (count) {
        var ary = [];
        var minDirection = -0.12, maxDirection = 0.12;
    ...
    var opts = sphere_opts.concat(generateRandomSpheres(2));
...
var renderer = function (gpuKernel, cpuKernel, gpuCanvas, cpuCanvas, scene) {
    var camera = scene.camera, lights = scene.lights, entities = scene.entities.map(function (f) { return f.toVector(); }), eyeVector = scene.eyeVector, vpRight = scene.vpRight, vpUp = scene.vpUp, canvasHeight = scene.canvasHeight, canvasWidth = scene.canvasWidth, fovRadians = scene.fovRadians, heightWidthRatio = scene.heightWidthRatio, halfWidth = scene.halfWidth, halfHeight = scene.halfHeight, cameraWidth = scene.cameraWidth, cameraHeight = scene.cameraHeight, pixelWidth = scene.pixelWidth, pixelHeight = scene.pixelHeight;
    var fps = {
        startTime: 0,
        frameNumber: 0,
        getFPS: function () {
    ...
    var moveEntity = function (width, height, depth, entity) {
        ...
        if (needsReflect) {
            _a = reflect(entity, normal), entity[15] = _a[0], entity[16] = _a[1], entity[17] = _a[2];
        }
        return entity;
        var _a;
    };
...
var createKernel = function (mode, scene) {
    ...
    return gpu.createKernel(function (camera, lights, entities, eyeVector, vpRight, vpUp, canvasHeight, canvasWidth, fovRadians, heightWidthRatio, halfWidth, halfHeight, cameraWidth, cameraHeight, pixelWidth, pixelHeight) {
        ...
        var maxEntityDistance = Math.pow(2, 64);
        var nearestEntityDistance = Math.pow(2, 64);
        for (var i = 0; i < this.constants.ENTITY_COUNT; i++) {
            if (entities[i][0] == this.constants.SPHERE) {
                var distance = sphereIntersection(entities[i][1], entities[i][2], entities[i][3], entities[i][7], rayPtX, rayPtY, rayPtZ, normRayVecX, normRayVecY, normRayVecZ);
