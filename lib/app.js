...
(function (Scene) {
    ...
    let opts = [sphere_1_opts, sphere_2_opts];
...
var renderer = (gpuKernel, cpuKernel, gpuCanvas, cpuCanvas, scene) => {
    let camera = scene.camera, lights = scene.lights, entities = scene.entities.map(f => f.toVector()), eyeVector = scene.eyeVector, vpRight = scene.vpRight, vpUp = scene.vpUp, canvasHeight = scene.canvasHeight, canvasWidth = scene.canvasWidth, fovRadians = scene.fovRadians, heightWidthRatio = scene.heightWidthRatio, halfWidth = scene.halfWidth, halfHeight = scene.halfHeight, cameraWidth = scene.cameraWidth, cameraHeight = scene.cameraHeight, pixelWidth = scene.pixelWidth, pixelHeight = scene.pixelHeight;
    var fps = {
        startTime: 0,
        frameNumber: 0,
        getFPS: function () {
...
var createKernel = (mode, scene) => {
    ...
    return gpu.createKernel(function (camera, lights, entities, eyeVector, vpRight, vpUp, canvasHeight, canvasWidth, fovRadians, heightWidthRatio, halfWidth, halfHeight, cameraWidth, cameraHeight, pixelWidth, pixelHeight) {
        ...
        this.color(colorX, colorY, colorZ);
