    ...
    var generateRandomSpheres = function (count) {
        ...
        for (var i = 0; i < count; i++) {
            ary.push({
                ...
                x: (i * 3.2) - 7, y: rand(0, 7), z: rand(-7, 2), radius: rand(0.3, 2.8),
...
var updateSlider = function (elem) {
    isRunning = false;
...
var renderer = function (gpuKernel, cpuKernel, gpuCanvas, scene) {
    ...
    var checkSphereSphereCollision = function (allSpheres) {
        for (var first = 0; first < allSpheres.length - 1; first++) {
            if (allSpheres[first][0] === Entity.Type.LIGHTSPHERE) {
                continue;
            }
            var sphere = allSpheres[first];
            for (var second = first + 1; second < allSpheres.length; second++) {
                if (allSpheres[second][0] === Entity.Type.LIGHTSPHERE) {
                    continue;
                }
                var other = allSpheres[second];
